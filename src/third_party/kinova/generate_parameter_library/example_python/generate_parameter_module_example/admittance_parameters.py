...
class admittance_controller:
    ...
    class Params:
        ...
        lt_eq_fifteen = 1
        gt_fifteen = 16
        gt_fifteen_lt_eq_twenty = 20
        gt_fifteen_lt_twenty = 16
        one_number = 14540
        three_numbers = [3, 4, 5]
    ...
    class ParamListener:
        ...
        def refresh_dynamic_parameters(self):
            ...
            for value_1 in updated_params.joints:
                for value_2 in updated_params.dof_names:
                    ...
                    if not self.node_.has_parameter(self.prefix_ + param_name):
                        descriptor = ParameterDescriptor(description=r"map parameter without struct name", read_only = False)
            ...
            for value_1 in updated_params.joints:
                for value_2 in updated_params.dof_names:
                    ...
                    updated_params.add_entry(value_1).add_entry(value_2)
                    entry = updated_params.get_entry(value_1).get_entry(value_2)
                    param_name = f"{self.prefix_}{value_1}.{value_2}.limit"
                    if not self.node_.has_parameter(self.prefix_ + param_name):
                        descriptor = ParameterDescriptor(description=r"specifies limit for x, y and z axis", read_only = False)
                        descriptor.floating_point_range.append(FloatingPointRange())
                        descriptor.floating_point_range[-1].from_value = -10.0
                    ...
                    if validation_result:
                        raise InvalidParameterValueException('__map_joints.__map_dof_names.limit',param.value, 'Invalid value set during initialization for parameter __map_joints.__map_dof_names.limit: ' + validation_result)
                    validation_result = ParameterValidators.lower_element_bounds(param, -10.0)
                    if validation_result:
                        raise InvalidParameterValueException('__map_joints.__map_dof_names.limit',param.value, 'Invalid value set during initialization for parameter __map_joints.__map_dof_names.limit: ' + validation_result)
                    validation_result = ParameterValidators.upper_element_bounds(param, 10.0)
                    if validation_result:
                        raise InvalidParameterValueException('__map_joints.__map_dof_names.limit',param.value, 'Invalid value set during initialization for parameter __map_joints.__map_dof_names.limit: ' + validation_result)
                    entry.limit = param.value
            ...
            for value_1 in updated_params.joints:
                for value_2 in updated_params.dof_names:
                    ...
                    if not self.node_.has_parameter(self.prefix_ + param_name):
                        descriptor = ParameterDescriptor(description=r"test nested map params", read_only = False)
                        descriptor.floating_point_range.append(FloatingPointRange())
                ...
                for value_2 in updated_params.dof_names:
                    for value_3 in updated_params.joints:
                        for value_4 in updated_params.dof_names:
                            ...
                            if not self.node_.has_parameter(self.prefix_ + param_name):
                                descriptor = ParameterDescriptor(description=r"test deep nested map params", read_only = False)
                                descriptor.floating_point_range.append(FloatingPointRange())
                                descriptor.floating_point_range[-1].from_value = 0.0001
                                descriptor.floating_point_range[-1].to_value = float('inf')
            ...
            for value_1 in updated_params.joints:
                ...
                if not self.node_.has_parameter(self.prefix_ + param_name):
                    descriptor = ParameterDescriptor(description=r"proportional gain term", read_only = False)
                    descriptor.floating_point_range.append(FloatingPointRange())
                    descriptor.floating_point_range[-1].from_value = 0.0001
                    descriptor.floating_point_range[-1].to_value = float('inf')
                    parameter = entry.p
            ...
            for value_1 in updated_params.joints:
                ...
                if not self.node_.has_parameter(self.prefix_ + param_name):
                    descriptor = ParameterDescriptor(description=r"integral gain term", read_only = False)
                    parameter = entry.i
                    self.node_.declare_parameter(param_name, parameter, descriptor)
                param = self.node_.get_parameter(param_name)
                self.logger_.debug(param.name + ": " + param.type_.name + " = " + str(param.value))
            ...
            for value_1 in updated_params.joints:
                ...
                if not self.node_.has_parameter(self.prefix_ + param_name):
                    descriptor = ParameterDescriptor(description=r"derivative gain term", read_only = False)
                    parameter = entry.d
                    self.node_.declare_parameter(param_name, parameter, descriptor)
                param = self.node_.get_parameter(param_name)
                self.logger_.debug(param.name + ": " + param.type_.name + " = " + str(param.value))
            ...
            for value_1 in updated_params.dof_names:
                ...
                if not self.node_.has_parameter(self.prefix_ + param_name):
                    descriptor = ParameterDescriptor(description=r"general gain", read_only = False)
                    parameter = entry.k
                    self.node_.declare_parameter(param_name, parameter, descriptor)
                param = self.node_.get_parameter(param_name)
                self.logger_.debug(param.name + ": " + param.type_.name + " = " + str(param.value))
            ...
            for value_1 in updated_params.nested_map.entries:
                ...
                if not self.node_.has_parameter(self.prefix_ + param_name):
                    descriptor = ParameterDescriptor(description=r"A value in the nested map with sibling keys", read_only = False)
        ...
        def update(self, parameters):
            ...
            for param in parameters:
                ...
                if param.name == self.prefix_ + "acceleration_limits":
                    validation_result = ParameterValidators.fixed_size(param, 3)
                    if validation_result:
                        return SetParametersResult(successful=False, reason=validation_result)
                    validation_result = ParameterValidators.lower_element_bounds(param, -10.0)
                    ...
                    if validation_result:
                        return SetParametersResult(successful=False, reason=validation_result)
                    updated_params.acceleration_limits = param.value
                    self.logger_.debug(param.name + ": " + param.type_.name + " = " + str(param.value))
                ...
                if param.name == self.prefix_ + "gt_fifteen_lt_eq_twenty":
                    validation_result = ParameterValidators.gt(param, 15)
                    if validation_result:
                        return SetParametersResult(successful=False, reason=validation_result)
                    validation_result = ParameterValidators.lt_eq(param, 20)
                    if validation_result:
                        return SetParametersResult(successful=False, reason=validation_result)
                    updated_params.gt_fifteen_lt_eq_twenty = param.value
                    self.logger_.debug(param.name + ": " + param.type_.name + " = " + str(param.value))

                if param.name == self.prefix_ + "gt_fifteen_lt_twenty":
                    validation_result = ParameterValidators.gt(param, 15)
                    if validation_result:
                        return SetParametersResult(successful=False, reason=validation_result)
                    validation_result = ParameterValidators.lt(param, 20)
                    if validation_result:
                        return SetParametersResult(successful=False, reason=validation_result)
                    updated_params.gt_fifteen_lt_twenty = param.value
                    self.logger_.debug(param.name + ": " + param.type_.name + " = " + str(param.value))
                    ...
                    for value_1 in updated_params.joints:
                        for value_2 in updated_params.dof_names:
                            ...
                            if param.name == param_name:
                                ...
                                updated_params.get_entry(value_1).get_entry(value_2).limit = param.value
                                self.logger_.debug(param.name + ": " + param.type_.name + " = " + str(param.value))
        ...
        def declare_params(self):
            ...
            if not self.node_.has_parameter(self.prefix_ + "scientific_notation_num"):
                descriptor = ParameterDescriptor(description=r"Test scientific notation", read_only = False)
            ...
            if not self.node_.has_parameter(self.prefix_ + "interpolation_mode"):
                descriptor = ParameterDescriptor(description=r"specifies which algorithm to use for interpolation.", read_only = False)
            ...
            if not self.node_.has_parameter(self.prefix_ + "subset_selection"):
                descriptor = ParameterDescriptor(description=r"test subset of validator.", read_only = False)
            ...
            if not self.node_.has_parameter(self.prefix_ + "joints"):
                descriptor = ParameterDescriptor(description=r"specifies which joints will be used by the controller", read_only = False)
            ...
            if not self.node_.has_parameter(self.prefix_ + "pid.rate"):
                descriptor = ParameterDescriptor(description=r"update loop period in seconds", read_only = False)
            ...
            if not self.node_.has_parameter(self.prefix_ + "nested_map.entries"):
                descriptor = ParameterDescriptor(description=r"Keys for nested mapped parameters", read_only = False)
            ...
            if not self.node_.has_parameter(self.prefix_ + "fixed_string"):
                descriptor = ParameterDescriptor(description=r"test code generation for fixed sized string", read_only = False)
            ...
            if not self.node_.has_parameter(self.prefix_ + "fixed_array"):
                descriptor = ParameterDescriptor(description=r"test code generation for fixed sized array", read_only = False)
            ...
            if not self.node_.has_parameter(self.prefix_ + "fixed_string_no_default"):
                descriptor = ParameterDescriptor(description=r"test code generation for fixed sized string with no default", read_only = False)
            ...
            if not self.node_.has_parameter(self.prefix_ + "command_interfaces"):
                descriptor = ParameterDescriptor(description=r"specifies which command interfaces to claim", read_only = True)
            ...
            if not self.node_.has_parameter(self.prefix_ + "state_interfaces"):
                descriptor = ParameterDescriptor(description=r"specifies which state interfaces to claim", read_only = True)
                parameter = rclpy.Parameter.Type.STRING_ARRAY
            ...
            if not self.node_.has_parameter(self.prefix_ + "chainable_command_interfaces"):
                descriptor = ParameterDescriptor(description=r"specifies which chainable interfaces to claim", read_only = True)
                parameter = rclpy.Parameter.Type.STRING_ARRAY
            ...
            if not self.node_.has_parameter(self.prefix_ + "kinematics.plugin_name"):
                descriptor = ParameterDescriptor(description=r"specifies which kinematics plugin to load", read_only = False)
                parameter = rclpy.Parameter.Type.STRING
            ...
            if not self.node_.has_parameter(self.prefix_ + "kinematics.plugin_package"):
                descriptor = ParameterDescriptor(description=r"specifies the package to load the kinematics plugin from", read_only = False)
                parameter = rclpy.Parameter.Type.STRING
            ...
            if not self.node_.has_parameter(self.prefix_ + "kinematics.base"):
                descriptor = ParameterDescriptor(description=r"specifies the base link of the robot description used by the kinematics plugin", read_only = False)
                parameter = rclpy.Parameter.Type.STRING
            ...
            if not self.node_.has_parameter(self.prefix_ + "kinematics.tip"):
                descriptor = ParameterDescriptor(description=r"specifies the end effector link of the robot description used by the kinematics plugin", read_only = False)
                parameter = rclpy.Parameter.Type.STRING
            ...
            if not self.node_.has_parameter(self.prefix_ + "kinematics.alpha"):
                descriptor = ParameterDescriptor(description=r"specifies the damping coefficient for the Jacobian pseudo inverse", read_only = False)
            ...
            if not self.node_.has_parameter(self.prefix_ + "kinematics.group_name"):
                descriptor = ParameterDescriptor(description=r"specifies the group name for planning with Moveit", read_only = False)
                parameter = rclpy.Parameter.Type.STRING
            ...
            if not self.node_.has_parameter(self.prefix_ + "ft_sensor.name"):
                descriptor = ParameterDescriptor(description=r"name of the force torque sensor in the robot description", read_only = False)
                parameter = rclpy.Parameter.Type.STRING
            ...
            if not self.node_.has_parameter(self.prefix_ + "ft_sensor.frame.id"):
                descriptor = ParameterDescriptor(description=r"frame of the force torque sensor", read_only = False)
                parameter = rclpy.Parameter.Type.STRING
            ...
            if not self.node_.has_parameter(self.prefix_ + "ft_sensor.frame.external"):
                descriptor = ParameterDescriptor(description=r"specifies if the force torque sensor is contained in the kinematics chain from the base to the tip", read_only = False)
            ...
            if not self.node_.has_parameter(self.prefix_ + "ft_sensor.filter_coefficient"):
                descriptor = ParameterDescriptor(description=r"specifies the coefficient for the sensor's exponential filter", read_only = False)
            ...
            if not self.node_.has_parameter(self.prefix_ + "control.frame.id"):
                descriptor = ParameterDescriptor(description=r"control frame used for admittance control", read_only = False)
                parameter = rclpy.Parameter.Type.STRING
            ...
            if not self.node_.has_parameter(self.prefix_ + "control.frame.external"):
                descriptor = ParameterDescriptor(description=r"specifies if the control frame is contained in the kinematics chain from the base to the tip", read_only = False)
            ...
            if not self.node_.has_parameter(self.prefix_ + "fixed_world_frame.frame.id"):
                descriptor = ParameterDescriptor(description=r"world frame, gravity points down (neg. Z) in this frame", read_only = False)
                parameter = rclpy.Parameter.Type.STRING
            ...
            if not self.node_.has_parameter(self.prefix_ + "fixed_world_frame.frame.external"):
                descriptor = ParameterDescriptor(description=r"specifies if the world frame is contained in the kinematics chain from the base to the tip", read_only = False)
            ...
            if not self.node_.has_parameter(self.prefix_ + "gravity_compensation.frame.id"):
                descriptor = ParameterDescriptor(description=r"frame which center of gravity (CoG) is defined in", read_only = False)
                parameter = rclpy.Parameter.Type.STRING
            ...
            if not self.node_.has_parameter(self.prefix_ + "gravity_compensation.frame.external"):
                descriptor = ParameterDescriptor(description=r"specifies if the center of gravity frame is contained in the kinematics chain from the base to the tip", read_only = False)
            ...
            if not self.node_.has_parameter(self.prefix_ + "gravity_compensation.CoG.pos"):
                descriptor = ParameterDescriptor(description=r"position of the center of gravity (CoG) in its frame", read_only = False)
            ...
            if not self.node_.has_parameter(self.prefix_ + "gravity_compensation.CoG.force"):
                descriptor = ParameterDescriptor(description=r"weight of the end effector, e.g mass * 9.81", read_only = False)
            ...
            if not self.node_.has_parameter(self.prefix_ + "admittance.selected_axes"):
                descriptor = ParameterDescriptor(description=r"specifies if the axes x, y, z, rx, ry, and rz are enabled", read_only = False)
            ...
            if not self.node_.has_parameter(self.prefix_ + "admittance.mass"):
                descriptor = ParameterDescriptor(description=r"specifies mass values for x, y, z, rx, ry, and rz used in the admittance calculation", read_only = False)
                descriptor.floating_point_range.append(FloatingPointRange())
                descriptor.floating_point_range[-1].from_value = 0.0001
            ...
            if not self.node_.has_parameter(self.prefix_ + "admittance.damping_ratio"):
                descriptor = ParameterDescriptor(description=r"specifies damping ratio values for x, y, z, rx, ry, and rz used in the admittance calculation. The values are calculated as damping can be used instead: zeta = D / (2 * sqrt( M * S ))", read_only = False)
                descriptor.floating_point_range.append(FloatingPointRange())
            ...
            if not self.node_.has_parameter(self.prefix_ + "admittance.stiffness"):
                descriptor = ParameterDescriptor(description=r"specifies stiffness values for x, y, z, rx, ry, and rz used in the admittance calculation", read_only = False)
                descriptor.floating_point_range.append(FloatingPointRange())
                descriptor.floating_point_range[-1].from_value = 0.0001
            ...
            if not self.node_.has_parameter(self.prefix_ + "acceleration_limits"):
                descriptor = ParameterDescriptor(description=r"specifies maximum acceleration limits for x, y and z axis", read_only = False)
                descriptor.floating_point_range.append(FloatingPointRange())
                descriptor.floating_point_range[-1].from_value = -10.0
                descriptor.floating_point_range[-1].to_value = 10.0
                parameter = updated_params.acceleration_limits
                self.node_.declare_parameter(self.prefix_ + "acceleration_limits", parameter, descriptor)

            if not self.node_.has_parameter(self.prefix_ + "enable_parameter_update_without_reactivation"):
                descriptor = ParameterDescriptor(description=r"if enabled, read_only parameters will be dynamically updated in the control loop", read_only = False)
            ...
            if not self.node_.has_parameter(self.prefix_ + "use_feedforward_commanded_input"):
                descriptor = ParameterDescriptor(description=r"if enabled, the velocity commanded to the admittance controller is added to its calculated admittance velocity", read_only = False)
            ...
            if not self.node_.has_parameter(self.prefix_ + "lt_eq_fifteen"):
                descriptor = ParameterDescriptor(description=r"should be a number less than or equal to 15", read_only = False)
            ...
            if not self.node_.has_parameter(self.prefix_ + "gt_fifteen"):
                descriptor = ParameterDescriptor(description=r"should be a number greater than 15", read_only = False)
                descriptor.integer_range.append(IntegerRange())
            ...
            if not self.node_.has_parameter(self.prefix_ + "gt_fifteen_lt_eq_twenty"):
                descriptor = ParameterDescriptor(description=r"should be a number greater than 15 and less than or equal to 20", read_only = False)
                descriptor.integer_range.append(IntegerRange())
                descriptor.integer_range[-1].from_value = 15
                descriptor.integer_range[-1].to_value = 20
                parameter = updated_params.gt_fifteen_lt_eq_twenty
                self.node_.declare_parameter(self.prefix_ + "gt_fifteen_lt_eq_twenty", parameter, descriptor)

            if not self.node_.has_parameter(self.prefix_ + "gt_fifteen_lt_twenty"):
                descriptor = ParameterDescriptor(description=r"should be a number greater than 15 and less than 20", read_only = False)
                descriptor.integer_range.append(IntegerRange())
                descriptor.integer_range[-1].from_value = 15
                descriptor.integer_range[-1].to_value = 20
                parameter = updated_params.gt_fifteen_lt_twenty
                self.node_.declare_parameter(self.prefix_ + "gt_fifteen_lt_twenty", parameter, descriptor)

            if not self.node_.has_parameter(self.prefix_ + "one_number"):
                descriptor = ParameterDescriptor(description=r"", read_only = True)
                descriptor.integer_range.append(IntegerRange())
            ...
            if not self.node_.has_parameter(self.prefix_ + "hover_override"):
                descriptor = ParameterDescriptor(description=r"Override hover action:\n    0: Hover\n    1: Push\n    2: Pull\n    -1: Do not override", read_only = False)
            ...
            if not self.node_.has_parameter(self.prefix_ + "angle_wraparound"):
                descriptor = ParameterDescriptor(description=r"For joints that wrap around (without end stop, ie. are continuous) where the shortest rotation to the target position is the desired motion. If true, the position error :math:`e = normalize(s_d - s)` is normalized between :math:`-\pi, \pi`. Otherwise  :math:`e = s_d - s` is used, with the desired position :math:`s_d` and the measured position :math:`s` from the state interface.", read_only = False)
            ...
            updated_params.admittance.stiffness = param.value
            param = self.node_.get_parameter(self.prefix_ + "acceleration_limits")
            self.logger_.debug(param.name + ": " + param.type_.name + " = " + str(param.value))
            validation_result = ParameterValidators.fixed_size(param, 3)
            if validation_result:
                raise InvalidParameterValueException('acceleration_limits',param.value, 'Invalid value set during initialization for parameter acceleration_limits: ' + validation_result)
            validation_result = ParameterValidators.lower_element_bounds(param, -10.0)
            if validation_result:
                raise InvalidParameterValueException('acceleration_limits',param.value, 'Invalid value set during initialization for parameter acceleration_limits: ' + validation_result)
            validation_result = ParameterValidators.upper_element_bounds(param, 10.0)
            ...
            updated_params.gt_fifteen = param.value
            param = self.node_.get_parameter(self.prefix_ + "gt_fifteen_lt_eq_twenty")
            self.logger_.debug(param.name + ": " + param.type_.name + " = " + str(param.value))
            validation_result = ParameterValidators.gt(param, 15)
            if validation_result:
                raise InvalidParameterValueException('gt_fifteen_lt_eq_twenty',param.value, 'Invalid value set during initialization for parameter gt_fifteen_lt_eq_twenty: ' + validation_result)
            validation_result = ParameterValidators.lt_eq(param, 20)
            if validation_result:
                raise InvalidParameterValueException('gt_fifteen_lt_eq_twenty',param.value, 'Invalid value set during initialization for parameter gt_fifteen_lt_eq_twenty: ' + validation_result)
            updated_params.gt_fifteen_lt_eq_twenty = param.value
            param = self.node_.get_parameter(self.prefix_ + "gt_fifteen_lt_twenty")
            self.logger_.debug(param.name + ": " + param.type_.name + " = " + str(param.value))
            validation_result = ParameterValidators.gt(param, 15)
            if validation_result:
                raise InvalidParameterValueException('gt_fifteen_lt_twenty',param.value, 'Invalid value set during initialization for parameter gt_fifteen_lt_twenty: ' + validation_result)
            validation_result = ParameterValidators.lt(param, 20)
            if validation_result:
                raise InvalidParameterValueException('gt_fifteen_lt_twenty',param.value, 'Invalid value set during initialization for parameter gt_fifteen_lt_twenty: ' + validation_result)
            updated_params.gt_fifteen_lt_twenty = param.value
